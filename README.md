...
- **华丽动画**：执行时显示金色奔跑的马动画，带发光特效，马的位置跟随进度条实时移动 ✨🐎🏇
- **精美进度条**：绿色渐变进度条，带高光和内阴影效果，视觉效果出众
- **SQL 语法高亮**：支持 SQL 代码语法高亮显示，提升可读性
- **行号精确对齐**：代码编辑器支持行号显示，使用 RichTextBox 实际行位置，完美对齐
...
### 5. 行号精确对齐
- 直接使用 RichTextBox.GetPositionFromCharIndex 获取实际行位置
- 逐行绘制，确保每行行号与代码完美对齐
- 使用与编辑器相同的字体和渲染方式
- ClearType 字体渲染，清晰美观
- 智能处理可见区域，避免过度绘制
